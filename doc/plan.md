...
|---|------------|------|
| 1 | 要件定義 | 機能・非機能要件の整理 |
| 2 | 設計 | アーキテクチャ・インターフェース・データ変換の設計 |
| 3 | プロジェクト準備 | Gradle・Spring Boot・proto 配置とコード生成 |
| 4 | コンバータ実装 | Tsurugi 型 ↔ Java 日時型の変換 |
| 5 | gRPC サービス実装 | 全 RPC の実装とサーバ起動 |
| 6 | 単体テスト | コンバータ・サービスロジックの単体テスト |
| 7 | 結合テスト | gRPC クライアントおよび Tsurugi からの呼び出し確認 |
